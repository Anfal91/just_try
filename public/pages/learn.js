...
function createOrbitLine(radius) {
    const orbitGeometry = new THREE.CircleGeometry(radius, 100);
    const orbitMaterial = new THREE.LineBasicMaterial({ color: 0xffffff, linewidth: 1 });
    const orbitLine = new THREE.LineLoop(orbitGeometry, orbitMaterial);
    orbitLine.rotation.x = -Math.PI / 2; // Align the orbit with the XY plane
    scene.add(orbitLine);
}
let btn = document.getElementById('btn');
btn.onclick = ()=>{
    // Create orbit lines for each planet
    createOrbitLine(50);   // Mercury
    createOrbitLine(75);   // Venus
    createOrbitLine(100);  // Earth
    createOrbitLine(125);  // Mars
    createOrbitLine(150);  // Jupiter
    createOrbitLine(175);  // Saturn
    createOrbitLine(200);  // Uranus
    createOrbitLine(225);  // Neptune
    createOrbitLine(250);  // Pluto
}
